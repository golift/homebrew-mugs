...
class Motifini < Formula
  ...
  homepage "{{URL}}"
  url "https://codeload.github.com/davidnewhall/motifini/tar.gz/v0.0.6"
  sha256 "076d8573bedb882bd05a410bafddadac51ebe5321baa00f321034bfd0263f779"
  head "{{URL}}"
  ...
  def install
    ...
    cd bin_path do
      system "make", "install", "VERSION=#{version}", "ITERATION=77", "PREFIX=#{prefix}", "ETC=#{etc}"
  ...
  def caveats
    <<-EOS
  Edit the config file at #{etc}/#{name}/motifini.conf then start #{name} with
  ...
  <plist version="1.0">
     <dict>
         ...
         <array>
             ...
             <string>#{etc}/#{name}/motifini.conf</string>
         </array>
         <key>RunAtLoad</key>
         <true/>
         <key>KeepAlive</key>
